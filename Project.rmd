...
#### Univariate plot
As Price is a quantitative continuous random variable, it can be visualised with a histogram (Figure  4).
...
#### Univariate plot
As  cross is a categorical variable, it can be seen on the following bar-chart (Figure 5).


```{r, warning = F, error = F, fig.cap = "Bar Chart of crossbreed status of advertised dogs on gumtree."}
...
#### Univariate plot
As  Pet Offered By is a categorical variable, it can be seen on the following bar-chart (Figure 6).


```{r, warning = F, error = F, fig.cap = "Bar-Chart of who is the pet offered by on gumtree."}
...
Here we produce a boxplot for pets offered by owners and those offered by breeders against the response variable price (Figure 13):
...
We now produce the same plot but against the natural logarithm of price, since the interquartile range spanned a small portion of the price axis when the outliers are considered (Figure 14).
...
Here we produce a boxplot for pets offered by owners and those offered by breeders against the response variable price (figure 15:
...
We can now filter out the NA values (Figure 16).
...
Again, we produce the same plot but against the natural logarithm of price, since the interquartile range spanned a small portion of the price axis when the outliers are considered (Figure 17).
...
### Vaccination Status
Now a boxplot is produced showing prices of dogs sorted by their vaccination status (Figure 18):
...
### Desexing Status
Here a boxplot is produced showing the prices of dogs sorted by their desexing status (Figure 21):
...
### State
Here we produce a boxplot of dog prices sorted by the state in which they are being sold (FIgure 24):
...
```{r, fig.cap = "Diagnostic Plots for Standard Model"}
tmp<-par(mfrow=c(2,2))
plot(test_model)
```
...
```{r, fig.cap = "Diagnostic Plots for Logarithm of Price Model"}
tmp<-par(mfrow=c(2,2))
plot(log_model)
