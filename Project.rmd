...
Firstly we need to clean by making sure all the values are able to be interpreted by R and thus able to be proerly analysed.

### Price
The price variable contains string inputs which need to be assigned to a numeric value and "NA" variables need to be set to the NA value R can interpret so that it knows the data for that subject is unavailable.
```{r}
table(gumtree$price)                        #checking the variable of price in the dataset
gumtree$price[gumtree$price == "NA"] <- NA  #seting all "NA" values to the NA understood by R
table(gumtree$price)                        #Check that worked 

```
Now R understands which subjects have data, and the ones that don't have been set to NA. 

```{r}
summary(gumtree$price)  #View the summary statistics so far
```
We see it's not concidering the price variable as quantitative data yet.

```{r}
class(gumtree$price)   #checking the class of variable R conciders price at this point

```
R conciders Price to be of class 'character'. We need to chance class to 'numeric' since Price is a quantitative variable so that R can do maths with these values. 
...
ggplot(gumtree, aes(price)) +
  geom_histogram(col = "black", fill = "Light Blue")  #plot of the price variable
```
...
```{r}
summary(gumtree$price)  #check the summary statics of Price
```

The prices seem reasonable, so we concidert the price variablbe clean. 


### Cross
Cross is a catagorical variable so we need to make sure R knows to treat is as a catagorical variable and to reset the string values of "NA" to the R value NA so R knows to not include that data in analysis.

```{r}
table(gumtree$cross)                        #Check the stauts of the values currently in R
gumtree$cross[gumtree$cross == "NA"] <- NA  #Set all string "NA" values to the R value NA
table(gumtree$cross)                        #Make sure that went through
class(gumtree$cross)                        #Check the class of the variables
gumtree$cross <- as.factor(gumtree$cross)   #set the class of the variable to factor

```
Now the data seems reasonable, we concider it cleaned. 

### Pet offered by
'Pet Offered By:' is also a catagorical variable so same as above we need to change the class to factor and change the "NA" values to the R recognisable NA values.
```{r}
table(gumtree$`Pet Offered By:`)                                      #Check the currnet status of the 'Pet Offered By:' values in R
gumtree$`Pet Offered By:`[gumtree$`Pet Offered By:` == "NA"] <- NA    #Set al the string "NA" values to NA in R
table(gumtree$`Pet Offered By:`)                                      #Check that the changed values are now NA
class(gumtree$`Pet Offered By:`)                                      #See what class"Pet Offered By:' is
gumtree$`Pet Offered By:` <- as.factor(gumtree$`Pet Offered By:`)     #set the class to factor 
summary(gumtree$`Pet Offered By:`)                                    #check the summary stats for good measure
```
We see no apparent anomalies in the 'Pet Offered By:' data, so we concider it cleaned.  
...
gumtree$micro[gumtree$micro == "NA"] <- NA    #set all the string "NA" values to NA in R
table(gumtree$micro)                          #Make sure those values have changed
...
```
The Microchipped data looks reasonable so we concider it cleaned. 
...
```
The vaccinated data looks ready to work with so we concider it cleaned.
...
```
The Desex data is now clean and ready to gleam!
...
```
We see the relinquished data is like a whistle, clean, ready for analysis!
...
We will now do a stupidity test for the Age variable by plotting the values in a histogram to see it seems legit.
...
These are much more reasonable values, and we concider the age variable cleaned.
...
```{r, warning = F, error = F, fig.cap = "Figure 1: Histogram of price of advertised dogs on gumtree in dollars."}
...
#### Univariate plot
As “cross” is a categorical variable, it can be visualised with a bar-chart (Fig. 2).
...
First, we create bivariate regression models of price against each predictor variable, to decide whether they have enough of an effect on price individually, and if not, they will not be considered for the final model.
...
Notice in the residual vs fitted plot, the points tend to be clustered to the left. This suggests that data is right skewed. Perhaps we can get a better model with a log transform of price. We apply the 3 algorithms we used previously. Since log(0) is infinite, we will do a +1 offset. This should be ok as the offset is small. 
...
```{r}
  log_full <-  log(price+1) ~ (age + cross + micro + vacc + desex + relinquished)^2
  smallest <- log(price+1) ~ 1
