...
```
We see it's not considering the price variable as quantitative data yet.

```{r}
class(gumtree$price)   #checking the class of variable R considers price at this point

```
R considers Price to be of class 'character'. We need to chance class to 'numeric' since Price is a quantitative variable so that R can do maths with these values. 
...
The prices seem reasonable, so we consider the price variablbe to be clean. 
...
```
Now the data seems reasonable, we consider it cleaned. 
...
```
We see no apparent anomalies in the 'Pet Offered By:' data, so we consider it cleaned.  
...
```
The Microchipped data looks reasonable so we consider it cleaned. 
...
```
The vaccinated data looks ready to work with so we consider it cleaned.
